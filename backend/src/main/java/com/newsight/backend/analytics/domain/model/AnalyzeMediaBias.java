// backend/src/main/java/com/newsight/backend/analytics/domain/model/AnalyzeMediaBias.java
...
@Entity
@Table(
        name = "T_ANALYZE_MEDIA_BIAS",
        uniqueConstraints = {
                @UniqueConstraint(
                        name = "UK_T_KBIAS__RUN_KMP",
...
@AllArgsConstructor
@Builder
@EqualsAndHashCode(of = "biasSeq")
@ToString(of = {"biasSeq", "trendRunSeq", "keywordSeq", "mediaCode", "periodFilter"})
public class AnalyzeMediaBias {

    @Id
    @GeneratedValue(strategy = GenerationType.IDENTITY)
    @Column(name = "BIAS_SEQ", nullable = false)
    private Long biasSeq;
    ...
    @Column(name = "BIAS_SCORE_TITLE", nullable = false, precision = 5, scale = 2)
    private BigDecimal biasScoreTitle;

    @Column(name = "BIAS_SCORE_CONTENT", nullable = false, precision = 5, scale = 2)
    private BigDecimal biasScoreContent;
