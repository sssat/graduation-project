...
# - 최신 TREND_RUN_SEQ
# - BASE_DATE
# - 요약 대상 키워드 목록(우선 FINAL_RANK(D30), 없으면 TREND_KEYWORD)
# - 키워드명
# - D30 윈도우에서 키워드별 기사 입력 후보(언론사별 최신 N)
...
def select_keywords_for_summary(*, conn, trend_run_seq: int, top_n: int) -> List[int]:
    """
    우선순위:
    1) T_TREND_KEYWORD_FINAL_RANK (D30 기준) 상위 N
...
def select_articles_for_keyword_d30(
    *,
    conn,
    trend_run_seq: int,
    keyword_seq: int,
