# data-pipeline/src/analyzer/sentiment/content/storage/content_sentiment_reader.py
# 본문 기반 감성분석 대상 기사 조회(Reader)
...
class ContentSentimentArticleRow:
    article_seq: int
    keyword_seq: int
    media_code: int
    content_clean: str
...
def fetch_content_clean_articles_for_sentiment(
    *,
    trend_run_seq: int,
    period_filter: str,
    keyword_seqs: Sequence[int],
) -> List[ContentSentimentArticleRow]:
    """
    감성분석 대상 기사 목록 조회
    - CONTENT_CLEAN이 빈 값이면 제외
    - 기간은 PUBLISHED_AT 기준
        ...
        with conn.cursor() as cur:
            ...
            cur.execute(
                ...
                SELECT
                  ...
                  CONTENT_CLEAN AS content_clean
                FROM T_NEWS_ARTICLE
                WHERE TREND_RUN_SEQ = %s
                  AND KEYWORD_SEQ IN ({placeholders})
                  AND PUBLISHED_AT >= %s
                  AND PUBLISHED_AT <  %s
                  AND CONTENT_CLEAN IS NOT NULL
                  AND TRIM(CONTENT_CLEAN) <> ''
                """,
                [int(trend_run_seq), *[int(x) for x in keyword_seqs], start_dt, end_dt],
            )
            rows = cur.fetchall() or []

            out: List[ContentSentimentArticleRow] = []
            for r in rows:
                out.append(
                    ContentSentimentArticleRow(
                        ...
                        content_clean=str(r["content_clean"] or ""),
                    )
                )
            return out
    finally:
