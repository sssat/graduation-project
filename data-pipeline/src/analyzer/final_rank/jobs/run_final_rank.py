...
def _parse_periods(raw: str) -> List[str]:
    """
    "TODAY,D7,D14,D30" 형태를 ["TODAY","D7","D14","D30"]로 파싱한다.
    - 공백 제거, 대문자 정규화
    - SUPPORTED_PERIODS 외 값은 에러 처리
    """
    s = (raw or "").strip()
    if not s:
        return ["TODAY", "D7", "D14", "D30"]
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help='기간 필터 목록(콤마 구분) 예: "TODAY,D7,D14,D30". 미지정 시 .env(FINAL_RANK_PERIODS) 사용',
    )
    parser.add_argument(
        "--refresh",
        action="store_true",
    ...
    periods_raw = args.periods if args.periods is not None else str(getattr(settings, "final_rank_periods", "TODAY,D7,D14,D30"))
